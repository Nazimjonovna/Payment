...
from .serializer import Paycomserializer
from auth import authentication
from .status import * 
from rest_framework.permissions import AllowAny


class MerchanApiView(APIView):
    ...
    VALIDATED_CLASS: Paycom = None
    reply = None
    ORDER_KEY = KEY = settings.PAYCOM_SETTINGS['ACCOUNT']['KEY']
    ...
    def __init__(self):
        ...
        super(MerchanApiView, self).__init__()

        def post(self, request):
            check = authentication(request)
            if check is False or not check:
                return Response(AUTH_ERROR)
            serializer = Paycomserializer(data = request.data, many=False)
    ...
    def create_transaction(self, validated_data):
        ...
        check_transaction = Payme.objects.filter(order_key=order_key).order_by('-id')
        if check_transaction.exists():
            transaction = check_transaction.first()
            if transaction.status != Payme.CANCELED and transaction._id == _id:
            ...
            elif transaction.status == Payme.PROCESSING:
                self.reply = dict(
    ...
    def check_transaction(self, validated_data):
        ...
        try:
            transaction = Payme.objects.get(_id=id)
    ...
    def cancel_transaction(self, validated_data):
        ...
        try:
            ...
            transaction.status = Payme.CANCELED
    ...
    def get_statement(self, validated_data):
        ...
        filtered_transactions = Payme.objects.filter(
    ...
    def response_check_transaction(self, transaction: Payme):
        self.reply = dict(result=dict(
